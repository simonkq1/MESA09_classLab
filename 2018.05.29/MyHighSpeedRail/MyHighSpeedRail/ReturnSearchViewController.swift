//
//  ReturnSearchViewController.swift
//  2018.05.29_app3
//
...
class ReturnSearchViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        let titileDate = titleDAteFormatter.string(from: ReturnSearchViewController.searchData["returnTime"] as! Date)
        title = "\(titileDate) 回程"
//        let a = parent as! SearchViewController
        let backItem = UIBarButtonItem(title: "重新查詢", style: .plain, target: nil, action: #selector(goBack))
        ...
        let mydata = ReturnSearchViewController.searchData
        let start = mydata["endStation"] as! String
        let end = mydata["startStation"] as! String
        ...
        for i in data {
            ...
            for (k, v) in i {
                ...
                if k == "ServiceDay" {
                    ...
                    let week = weekformatter.string(from: ReturnSearchViewController.searchData["returnTime"] as! Date)
...
//                    print(serviceDay["Friday"] as! Int)
                ...
                if k == "StopTimes" {
                    ...
                    for j in list {
                        ...
                        let searchDate = ReturnSearchViewController.searchData["returnTime"] as! Date
                        let searchDateB = formatter.string(from: searchDate)
                        let newSearchDate = (formatter.date(from: searchDateB))! + (8 * 60 * 60)
                        
                        let start = (ReturnSearchViewController.searchData["endStation"] as! String)
                        ...
                        //                        print(newDate)
                        let end = (ReturnSearchViewController.searchData["startStation"] as! String)
...
//        print("\(sorted.count) : \(indexPath.section)")
        ...
        trainNo.text = (sorted[indexPath.row]["trainNo"]?.hasPrefix("0"))! ? String(a) : sorted[indexPath.row]["trainNo"]
